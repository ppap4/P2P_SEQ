...
class FlattenedWindowMapping(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, batch_size: int, sparse_shape: list):
        ...
        # print(torch.max(mappings["flat2win"]), torch.max(mappings["win2flat"]), mappings["flat2win"].size(0),
        #       mappings["win2flat"].size(0), torch.max(coords[:, 0]),batch_size)
        # if mappings["win2flat"].size(0) < torch.max(mappings["flat2win"]):
        #     asd = 1
        return mappings
...
class PatchMerging3D(nn.Module):
    ...
    def forward(self, x, coords_shift=1, diffusion_scale=4):
        ...
        if x_merge.batch_size <= torch.max(x_merge.indices[:, 0]):
            print("downsample_list bs wrong")
...
class LIONBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, enc in enumerate(self.encoder):
            ...
            if x.batch_size <= torch.max(x.indices[:,0]):
                print("downsample_list bs wrong")
...
@MODELS.register_module()# for waymo and nuscenes, kitti, once
class LION3DBackboneOneStride(nn.Module):
    def __init__(self, model_cfg, points_features,
                 point_cloud_range,
                 voxel_size,
                 grid_size,
                 output_channels,
        ...
        self.dow2 = PatchMerging3D(self.layer_dim[1], self.layer_dim[1], down_scale=[2, 2, 2],
                                   norm_layer=norm_fn, diffusion=diffusion, diff_scale=diff_scale)
        ...
        self.dow3 = PatchMerging3D(self.layer_dim[2], self.layer_dim[3], down_scale=[2, 2, 2],
                                   norm_layer=norm_fn, diffusion=diffusion, diff_scale=diff_scale)
        ...
        self.dow4 = PatchMerging3D(self.layer_dim[3], self.layer_dim[3], down_scale=[2, 2, 2],
                                   norm_layer=norm_fn, diffusion=diffusion, diff_scale=diff_scale)
        ...
        self.conv_out = ConvModule(128 * 2, output_channels, 1, norm_cfg=norm_cfg)

        # self.num_filters = self.model_cfg.num_filters
        # in_channels = 3
        # feat_channels = [in_channels] + list(self.num_filters)
        # pfn_layers = []
    ...
    def forward(self, voxel_features, voxel_coords):
        ...
        # voxel_features, voxel_coords = self.voxel_encoder(feats, coords)
        # voxel_features = batch_dict['voxel_features']
        # voxel_coords = batch_dict['voxel_coords']
        batch_size = 128 * 2

        x = spconv.SparseConvTensor(
            features=voxel_features.to(torch.float32),
            indices=voxel_coords.int(),
...
@MODELS.register_module()# for argoverse
class LION3DBackboneOneStride_Sparse(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        x = spconv.SparseConvTensor(
            features=torch.tensor(voxel_features, dtype=torch.float32),
            indices=voxel_coords.int(),
            spatial_shape=self.sparse_shape,
            batch_size=batch_size
        )
...
class PFNLayer(nn.Module):
    ...
    def forward(self,
                ...
                aligned_distance: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Forward function.
        Args:
